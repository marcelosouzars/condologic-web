...
class _CondominiosScreenWebState extends State<CondominiosScreenWeb> {
  ...
  Future<void> _carregarDados() async {
    ...
    try {
      ...
      // 1. Carrega os Condomínios
      final dadosCondo = await _apiService.getCondominios(usuarioId: userId, nivel: nivel);
      
      // 2. Carrega os Usuários para filtrar apenas os Síndicos (Se for Master)
  ...
  Future<void> _salvarCondominio({int? idEdicao}) async {
    try {
      ...
      // Conversão matemática blindada contra uso de vírgulas
  ...
  void _abrirModal({Map<String, dynamic>? item}) {
    ...
    if (item != null) {
      ...
      // Tenta achar o síndico na lista pelo nome para preencher o Dropdown na edição
      try {
