...
class _MainWebScreenState extends State<MainWebScreen> {
          ...
          setState(() {
            ...
            if (_meusCondominios.isNotEmpty) {
              ...
              if (!isMaster && tenantIdDoPerfil != null) {
                ...
                int? ultimoVisto = _usuarioLogado?['tenant_id_sessao']; // Usamos uma chave separada para não sujar o perfil
                if (ultimoVisto != null) {
                   try {
  ...
  Widget build(BuildContext context) {
    ...
    int tenantIdAtivo = _condominioSelecionado?['id'] ?? 1;
        ...
        actions: [
          ...
          Center(
            child: ActionChip(
              ...
              label: Text("${_usuarioLogado?['nome'] ?? 'Usuário'} ${isMaster ? '(Master)' : ''}", style: TextStyle(color: Colors.blue[900], fontWeight: FontWeight.bold)),
